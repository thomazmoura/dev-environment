...
| Module | Purpose |
|---|---|
| `agent-radar/` | Detects which coding agent is waiting on you, by reading each pane's screen (`prefix + t` then `a`/`A`) |
...
| `scripts/` | Your own scripts, versioned and run from a picker (`prefix + s`) |
